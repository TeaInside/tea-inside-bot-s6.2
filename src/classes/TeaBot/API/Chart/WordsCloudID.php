...
class WordsCloudID
{
	 ...
	 * @return void
	 */
	public static function generateWordCloud(string $groupId)
	{
		$pdo = DB::pdo();
		$st = $pdo->prepare("
			SELECT `a`.`id`, `a`.`text` FROM `groups_messages` AS `a`
			LEFT JOIN `id_1n_words_cloud` AS `b`
		...
		while ($r = $st->fetch(PDO::FETCH_NUM)) {
			...
			foreach ($r[1] as $v) {
				if (
					(strlen($v) <= 3) ||
					(strlen($v) > 64) || 
					(array_search($v, self::STOPWORDS) !== false) ||
					(array_search($v, self::ADD_STOPWORDS) !== false)) {
					continue;
				}
				$hasWord = true;
